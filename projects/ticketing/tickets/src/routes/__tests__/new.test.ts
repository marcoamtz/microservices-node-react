...
it("returns an error if an invalid price is provided", async () => {
  await request(app).post("/api/tickets").set("Cookie", signin()).send({ title: "sdfhsdf", price: -10 }).expect(400);
  await request(app).post("/api/tickets").set("Cookie", signin()).send({ title: "fdsajfd" }).expect(400);
});
...
it("created a ticket with valid inputs", async () => {
  ...
  await request(app).post("/api/tickets").set("Cookie", signin()).send({ title, price: 20 }).expect(201);
